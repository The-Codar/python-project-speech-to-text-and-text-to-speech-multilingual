<!--
##########
Author: The-Codar (एniruddh वीarun)
Issued date: 22-11-2023
License: MIT License
Version: v1.0
##########
-->

<div style="width: 100%;">
  <img src="custom-template.svg" style="width: 100%;" alt="Click to see the source">
</div>

<br />

# **Speech to Text** & **Text to Speech** with **multilingual** recognition project in python.

### ➜ **Programming Languages Used:**
![python](https://img.shields.io/badge/Python-FFD43B?style=for-the-badge&logo=python&logoColor=blue) ![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)

### ➜ **License:**
[![License: MIT](https://img.shields.io/badge/License-MIT-brightgreen.svg)](https://opensource.org/licenses/MIT)

### ➜ **Version:**
[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)](https://github.com/yourusername/yourrepository/releases/tag/v1.0.0)

A simple and easy to use ***speech to text*** & ***text to speech*** with ***multi-language*** feature project made by using python.


> 📝 **IMPORTANT NOTE:** <br> 
❒ This is just a prototype project but you can use it to build many big projects thank you! <br>
❒ Easy to use user freindly code.

## ✻ **Project Description:**
Introducing my first python prototype project. In this project we can convert our voice into text and text to computer's voice also it can translate the text in <mark style="background-color: #fa4545; color: #ffffff; padding: 1px; border-radius: 5px;">134+ languages.</mark>

### ➜ **Supported Languages with there Language code list:**

| **Sr.No.** | **Language**                  | **ISO-639 Language code** |
|------------|-------------------------------|---------------------------|
|1           | Afrikaans                     | af                        |
|2           | Albanian                      | sq                        |
|3           | Amharic                       | am                        |
|4           | Arabic                        | ar                        |
|5           | Armenian                      | hy                        |
|6           | Assamese                      | as                        |
|7           | Aymara                        | ay                        |
|8           | Azerbaijani                   | az                        |
|9           | Bambara                       | bm                        |
|10          | Basque                        | eu                        |
|11          | Belarusian                    | be                        |
|12          | Bengali                       | bn                        |
|13          | Bhojpuri                      | bho                       |
|14          | Bosnian                       | bs                        |
|15          | Bulgarian                     | bg                        |
|16          | Catalan                       | ca                        |
|17          | Cebuano                       | ceb                       |
|18          | Chinese (Simplified)          | zh-CN or zh (BCP-47)      |
|19          | Chinese (Traditional)         | zh-TW (BCP-47)            |
|20          | Corsican                      | co                        |
|21          | Croatian                      | hr                        |
|22          | Czech                         | cs                        |
|23          | Danish                        | da                        |
|24          | Dhivehi                       | dv                        |
|25          | Dogri                         | doi                       |
|26          | Dutch                         | nl                        |
|27          | English                       | en                        |
|28          | Esperanto                     | eo                        |
|29          | Estonian                      | et                        |
|30          | Ewe                           | ee                        |
|31          | Filipino (Tagalog)            | fil                       |
|32          | Finnish                       | fi                        |
|33          | French                        | fr                        |
|34          | Frisian                       | fy                        |
|35          | Galician                      | gl                        |
|36          | Georgian                      | ka                        |
|37          | German                        | de                        |
|38          | Greek                         | el                        |
|39          | Guarani                       | gn                        |
|40          | Gujarati                      | gu                        |
|41          | Haitian Creole                | ht                        |
|42          | Hausa                         | ha                        |
|43          | Hawaiian                      | haw                       |
|44          | Hebrew                        | he or iw                  |
|45          | Hindi                         | hi                        |
|46          | Hmong                         | hmn                       |
|47          | Hungarian                     | hu                        |
|48          | Icelandic                     | is                        |
|49          | Igbo                          | ig                        |
|50          | Ilocano                       | ilo                       |
|51          | Indonesian                    | id                        |
|52          | Irish                         | ga                        |
|53          | Italian                       | it                        |
|54          | Japanese                      | ja                        |
|55          | Javanese                      | jv or jw                  |
|56          | Kannada                       | kn                        |
|57          | Kazakh                        | kk                        |
|58          | Khmer                         | km                        |
|59          | Kinyarwanda                   | rw                        |
|60          | Konkani                       | gom                       |
|61          | Korean                        | ko                        |
|62          | Krio                          | kri                       |
|63          | Kurdish                       | ku                        |
|64          | Kurdish (Sorani)              | ckb                       |
|65          | Kyrgyz                        | ky                        |
|66          | Lao                           | lo                        |
|67          | Latin                         | la                        |
|68          | Latvian                       | lv                        |
|69          | Lingala                       | ln                        |
|70          | Lithuanian                    | lt                        |
|71          | Luganda                       | lg                        |
|72          | Luxembourgish                 | lb                        |
|73          | Macedonian                    | mk                        |
|74          | Maithili                      | mai                       |
|75          | Malagasy                      | mg                        |
|76          | Malay                         | ms                        |
|77          | Malayalam                     | ml                        |
|78          | Maltese                       | mt                        |
|79          | Maori                         | mi                        |
|80          | Marathi                       | mr                        |
|81          | Meiteilon (Manipuri)          | mni-Mtei                  |
|82          | Mizo                          | lus                       |
|83          | Mongolian                     | mn                        |
|84          | Myanmar (Burmese)             | my                        |
|85          | Nepali                        | ne                        |
|86          | Norwegian                     | no                        |
|87          | Nyanja (Chichewa)             | ny                        |
|88          | Odia (Oriya)                  | or                        |
|89          | Oromo                         | om                        |
|90          | Pashto                        | ps                        |
|91          | Persian                       | fa                        |
|92          | Polish                        | pl                        |
|93          | Portuguese (Portugal, Brazil) | pt                        |
|94          | Punjabi                       | pa                        |
|95          | Quechua                       | qu                        |
|96          | Romanian                      | ro                        |
|97          | Russian                       | ru                        |
|98          | Samoan                        | sm                        |
|99          | Sanskrit                      | sa                        |
|100         | Scots Gaelic                  | gd                        |
|101         | Sepedi                        | nso                       |
|102         | Serbian                       | sr                        |
|103         | Sesotho                       | st                        |
|104         | Shona                         | sn                        |
|105         | Sindhi                        | sd                        |
|106         | Sinhala (Sinhalese)           | si                        |
|107         | Slovak                        | sk                        |
|108         | Slovenian                     | sl                        |
|109         | Somali                        | so                        |
|110         | Spanish                       | es                        |
|111         | Sundanese                     | su                        |
|112         | Swahili                       | sw                        |
|113         | Swedish                       | sv                        |
|114         | Tagalog (Filipino)            | tl                        |
|115         | Tajik                         | tg                        |
|116         | Tamil                         | ta                        |
|117         | Tatar                         | tt                        |
|118         | Telugu                        | te                        |
|119         | Thai                          | th                        |
|120         | Tigrinya                      | ti                        |
|121         | Tsonga                        | ts                        |
|122         | Turkish                       | tr                        |
|123         | Turkmen                       | tk                        |
|124         | Twi (Akan)                    | ak                        |
|125         | Ukrainian                     | uk                        |
|126         | Urdu                          | ur                        |
|127         | Uyghur                        | ug                        |
|128         | Uzbek                         | uz                        |
|129         | Vietnamese                    | vi                        |
|130         | Welsh                         | cy                        |
|131         | Xhosa                         | xh                        |
|132         | Yiddish                       | yi                        |
|133         | Yoruba                        | yo                        |
|134         | Zulu                          | zu                        | 

## ✻ **Getting Started**

### ➜ **Dependencies**

* #### Required Commands to install python dependencies for this poject use terminals like windows cmd, `windows powershell`, `mac terminal`, `linux terminal` etc...:

    a) Installing `googletrans` module: <br />
        - googletrans module src link: https://pypi.org/project/googletrans/3.1.0a0/ <br />
        - Short Description: Googletrans is a free and unlimited python library that implemented Google Translate API. This uses the Google Translate Ajax API to make calls to such methods as detect and translate. <br />
        - Author: SuHun Han <br />
        - License: Freeware (MIT)

    ```python
    pip install googletrans==3.1.0a0
    ```

    b) Installing `SpeechRecognition` module: <br />
        - SpeechRecognition module src link: https://pypi.org/project/SpeechRecognition/ <br />
        - Short Description: Library for performing speech recognition, with support for several engines and APIs, online and offline. <br />
        - Author: Anthony Zhang (Uberi) <br />
        - License: BSD License (BSD)

    ```python
    pip install SpeechRecognition
    ```

    c) Installing `PyAudio` module: <br />
        - PyAudio module src link: https://pypi.org/project/PyAudio/ <br />
        - Short Description: PyAudio provides Python bindings for PortAudio v19, the cross-platform audio I/O library. With PyAudio, you can easily use Python to play and record audio on a variety of platforms, such as GNU/Linux, Microsoft Windows, and Apple macOS. <br />
        - Author: Hubert Pham <br />
        - License: MIT License (MIT)

    ```python
    pip install PyAudio
    ```

    d) Installing `gTTS` module: <br />
        - gTTS module src link: https://pypi.org/project/gTTS/ <br />
        - Short Description: gTTS (Google Text-to-Speech), a Python library and CLI tool to interface with Google Translate's text-to-speech API. Write spoken mp3 data to a file, a file-like object (bytestring) for further audio manipulation, or stdout. http://gtts.readthedocs.org/ <br />
        - Author: Pierre Nicolas Durette <br />
        - License: MIT License (MIT)

    ```python
    pip install gTTS
    ```

    e) Installing `playsound==1.2.2` module: <br />
        - playsound==1.2.2 module src link: https://pypi.org/project/playsound/1.2.2/ <br />
        - Short Description: Pure Python, cross platform, single function module with no dependencies for playing sounds. <br />
        - Author: Taylor Marks <br />
        - License: MIT License (MIT)

    ```python
    pip install playsound==1.2.2
    ```
* #### Optional:

    a) Installing `pyttsx3` module: <br />
        - pyttsx3 module src link: https://pypi.org/project/pyttsx3/ <br />
        - Short Description: pyttsx3 is a text-to-speech conversion library in Python. Unlike alternative libraries, it works offline, and is compatible with both Python 2 and 3. <br />
        - Author: Natesh M Bhat <br />
        - License: GNU General Public License v3 (GPLv3)

    ```python
    pip install pyttsx3
    ```

    b) Installing `python-vlc` module: <br />
        - python-vlc module src link: https://pypi.org/project/python-vlc/ <br />
        - Short Description: This module provides ctypes-based bindings. http://wiki.videolan.org/PythonBinding for the native libvlc API ( see http://wiki.videolan.org/LibVLC ) of the VLC video player. Note that it relies on an already present install of VLC. It has been automatically generated from the include files of vlc 3.0.20, using generator 1.23. <br />
        - Author: Olivier Aubert <br />
        - License: GNU Lesser General Public License v2 or later (LGPLv2+) (LGPL-2.1+)

    ```python
    pip install python-vlc
    ```

## ✻ **Project Author/Owner Info:**
* visit github profile: https://github.com/The-Codar
* github username: The-Codar
* Author/Owner: एniruddh वीarun
* Issued Date: 22-11-2023
* Project License: MIT License (Anyone can use!)
* Version: v1.0

## ✻ **Acknowledgments:**

### ➜ **Inspirations:**

  - [codewithharry](https://github.com/codewithharry)
  - [WsCube Tech](https://youtu.be/S63BPU_bcrU?si=8cycL8kK2Qk7I6Q_)

<h3 style="text-align: center; font-weight: 600;"> Feel free to give me feedback related to my projects. feedback will be most welcome and appreciable! <br /> Thanks for reading. Have a great day! </h3>